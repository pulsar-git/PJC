...
/** \example test.cpp
    There is an example of the usage of this class.
*/
...
namespace Leo
{
  ...
  public:
      ...
      typedef enum Type 
	{
      ...
      Type type; ///< the type of th element
      
      Element( Type type = UNDEF, std::string section = "", std::string key = "", std::string value = "" )
...
namespace Leo
{
  ...
  template<class T>std::string x2s( T what ) ///< converts whatever to a string
  {
    std::strstream str;
  ...
  template <class T> T s2x( const std::string& s ) ///< extracts everything from a string
  {
    std::strstream str;
...
extern "C" {
  /// library identify check function, used bei autoconf/automake
  /*** For autoconf check add this line to your configure.in file:
       AC_CHECK_LIB(LeoIni, leoini, echo -n;, echo not found ;exit, -lstdc++)
  */
  char leoini();

  /*** For autoconf check add this line to your configure.in file:
       AC_CHECK_LIB(LeoIni, leoini_version_2_2, echo "LeoIni >= 2.1.1 (yes)", \
       echo "LeoIni >= 2.1.1 (no)"; exit, -lstdc++ )
  */
  char leoini_version_2_1_1();
}
